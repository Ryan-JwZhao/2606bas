...
class BallCenterGeometry:
    ...
    def locate(
        ...
        *,
        ...
        if self._calibration.ball_compensation_model.is_valid:
        ...
        method_weight = (
            1.0
            if method.startswith("segmentation_ellipse")
            else 0.90
            if method.startswith("appearance_ellipse")
            else 0.25
            if method.startswith("bbox")
            else 0.75
        )
        support_factor = 0.35 + 0.65 * float(np.clip(support, 0.0, 1.0))
        uncertainty_factor = float(np.clip(radius_mm / max(radius_mm + uncertainty, 1e-6), 0.25, 1.0))
        reliability = float(np.clip(method_weight * support_factor * uncertainty_factor, 0.0, 1.0))
