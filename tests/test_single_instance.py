...
from bas import cli
from bas import single_instance
...
def test_runtime_single_instance_rejects_second_acquire() -> None:
    mutex_name = f"Local\\BAS_TEST_{uuid.uuid4().hex}"
    first = single_instance._acquire_windows_mutex(mutex_name)
    assert first is not None
    try:
        second = single_instance._acquire_windows_mutex(mutex_name)
        assert second is None
    finally:
        first.release()

    third = single_instance._acquire_windows_mutex(mutex_name)
