...
- 采集器按“完整一杆”写一条 JSONL，不是每帧写一条。
- 只有状态机识别到 `SHOT_STARTED`，并且后续进入 `TURN_RESOLVE` 或 `ANOMALY`，这一杆才会落成一条样本。
- 现在改成了“首次真正写入样本时再创建文件”，所以没有识别到完整击球时，不会再留下空的 `0KB` 样本文件。
- 如果检测/跟踪在击球过程中频繁丢球、重复生成 `cue`、大量出现 `ANOMALY`，样本数量和标签质量都会明显下降。
- 建议采样时把 `detect_fps_limit_hz` 调高到接近相机帧率，或者先设为 `0` 取消限速，优先保证状态机能稳定识别开杆和收杆。
