...
| 状态机 | Modern | 事件证据、逐球进洞 FSM、重现反证和无人值守自动结算 |
...
- 强证据要求 interior、throat 或 visual inward 至少一个成立，且不能有 visual outward；仅 projected entry 不足以确认。
- `detected` 不是看到球消失就立即发出，而是在重现窗口、袋唇反证和出袋反证均未命中后发出。

### 3.1.6 反证、撤回与无人值守处理

- 同一 track 重现：拒绝候选；若球组身份变化也自动拒绝并记录原因。
- 其他 track 在同袋附近重现且位置/时间可对应：跨 ID 重现反证。
- 投影入袋后轨迹反向并持续至少约 90 ms，或立即达到明显向外速度：拒绝。
- 候选未完成又回到普通台面区域：拒绝。
- 袋口改变、长期停在袋唇、证据不足或身份关联歧义：自动拒绝，不进入人工等待。
- track ID 在袋前切换时，可按时间、深度、横向投影和球组完成轨迹接力，避免把同一颗高速球拆成两颗。

### 3.1.7 与比赛状态的提交

`POCKET_COMMIT_READY` 进入 shot context 后，状态机仍要等待回合结束并结合：
...
观测对账层只在“稳定可见数量高于账本”这一可证明误删场景自动恢复账本，并同步驳回本杆冲突的进洞；
“稳定可见数量低于账本”只提供当杆有效视图，不凭消失直接永久扣球。随后统一生成裁判意图与最终
`POCKET_CONFIRMED`，不进入人工等待。这样把“视觉事实”、“进球事实”和“比赛规则”分成三个层级。

### 3.1.8 值得 Highlight：负证据优先的可撤回判定

算法不仅累积“像进球”的证据，还显式等待“又出来了、停在袋唇、方向反了、换袋了、身份冲突”
等负证据。`candidate -> tentative -> commit_ready -> confirmed/rejected` 的分层事件让状态机能够等待反证、
自动定案并保留原因码，明显强于“球靠近袋口后消失 N 帧即进球”的单阈值方案。
...
- 当前四分类规则模型没有球杆类别，运行时主要从白球附近的帧边缘拟合球杆无向轴；轴线到白球中心的垂直距离与杆尖间隙分别评分，避免库边或投影网格抢占方向。
- 当模型提供 `cue_stick` track 时，边缘线必须与检测区域形成足够长的重叠；分割 mask 会用 PCA 生成定向端点，轴对齐 bbox 仅作为最终回退。
- 方向解析比较两种朝向，以白球为锚点，根据杆身前后支撑长度消除 180° 歧义。低于物理像素门槛的两侧差异保持模糊状态，不进入方向历史。
- 无向轴角度和正反符号分开稳定；所有大角度跳变均经过确认帧门槛。稳定结果在同一帧共享给 target shot、target lock、cue sector 和预览。
