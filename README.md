...
- 规则模式和自由模式共用 `bas/planning/cue_aim.py` 的球杆瞄准线检测：优先从当前画面边缘识别真实杆线，失败时再回退到 `cue_stick` 检测框，改善斜杆或检测框方向不可靠时球杆扇形不生效的问题。
- 球杆扇形切换现在会在确认帧数内持有上一条仍然有效的路线；如果杆线短时抖到对方球方向，调试信息会显示 `cue_sector_policy=stable_hold`，不会立即把画线切过去。
...
2. 在主界面“设置 -> 路线策略”确认“启用球杆扇形二次纠正”已开启。
3. 现场可用 Stream Deck 的 `toggle-target-group` 在 `solid` / `stripe` 之间切换当前目标组；若当前组已稳定清零，系统会自动推荐黑球。
4. 排查时查看状态调试信息里的 `turn_target_group`、`raw_turn_target_group`、`target_resolution`、`visible_group_counts` 和 `cue_sector_policy`。

## 2026-06-28 设置窗口分组优化
- 设置窗口不再把全部参数集中在一个“基础”页里，已拆成 `相机采集`、`检测状态`、`标定几何`、`投影输出`、`路线策略`、`学习数据`、`投影修正`、`颗星公式` 等子标签页。
- 常用现场调参入口更聚焦：相机相关参数在 `相机采集`，模型/状态机在 `检测状态`，路径与标定文件在 `标定几何`，路线冻结和球杆扇形纠正在 `路线策略`。
...
- 默认只有所有球都静止时才启用扇形纠正；静止判断只检查 `cue/solid/stripe/black`，不要求 `cue_stick` 静止，方便球杆进入瞄准时实时触发。
- 如果当前回合是纯色/花色：
...
2. 使用规则模式画线。
3. 打开“设置 -> 路线策略 -> 球杆扇形二次纠正”，可调整：
   - `启用球杆扇形二次纠正`
   - `扇形总夹角(度)`：填写总夹角，系统自动左右平分。
   - `边缘剔除(度)`：用于剔除靠近两侧边界的球，降低闪烁。
   - `只在所有球静止时生效`：默认开启；关闭后球未完全静止时也允许扇形纠正。
