...
移动端页面按手机竖屏组织，组件宽度随屏幕变化：顶部显示连接状态，全宽视频保持 16:9，右下角按钮可进入横屏纯视频全屏。页面提供中式八球/追分前端模式选择，以及纯色球/花色球切换、颗星公式、长期规则/自由模式、下一杆自由/黑球和精彩瞬间录制控制。“切换花色”只会把状态机当前已确定的纯色球/花色球切换到另一花色，不会主动指定尚未确定的初始花色。单杆设定会显示独立状态条，可随时取消并恢复长期规则与状态机；追分模式选择仍是前端模式切换，但点选目标球与中式八球使用同一规划入口。
...
PWA 安装需要 HTTPS 安全上下文（或在本机 `localhost` 调试）；当前默认的 `http://<局域网 IP>:17070` 仍可正常使用普通 Web 模式，但手机浏览器可能不会提供 PWA 安装入口。实际部署时可使用 HTTPS 反向代理或可信证书终止 TLS，再访问相同页面并选择“添加到主屏幕/安装应用”。PWA service worker 只缓存页面壳资源，不会缓存实时 MJPEG 或 `/api/*` 数据。
...
打开局域网 Web 控制页后，直接点击实时视频中的非白球即可指定进洞规划目标。该能力同时适用于中式八球与追分前端模式，以及规则模式与自由模式；自由模式下，手动点选目标会优先生成该目标的进洞路线。
