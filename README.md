...
训练验证会检查袋口标定、所需球是否齐全、是否存在项目外的球、白球是否可见，以及一字线/双排项目的队形。袋口标定不可用时训练不允许开始，不再把普通的持续消失降级当作进球。点击“开始验证”后，编号跟踪器只负责提供球号身份；进球事实统一交给规则模式的 `PerBallPocketFSM` 判定。训练分支现在与 Modern 规则模式共享完整台面几何、袋前轨迹、跨 ID 接力、袋口视觉观察和重现反证窗口，只有收到高置信 `POCKET_DETECTED` 才把对应号码记为进球，不再用“最后可见位置距离袋口约 190 mm”单独推断。因此高速球在进入严格袋区前被遮挡时，可以依据合法入袋轨迹继续确认，不会抢先报“球在非袋口失踪”。袋口观察器在训练模式只接收当前项目要求的号码和白球，偶发识别出的项目外球号不会抢占袋口关联。

`training.disappearance_confirm_frames` 仍用于识别没有任何有效袋口候选的持续失踪，但只在编号跟踪器已经彻底释放该球后才开始计数。检测缓存帧、跟踪器仍持有的 `occluded` 帧以及袋口视觉仍报告 `lip_occupied` 的帧都不会累加，因此白球或目标球静止在袋唇、被袋角短暂遮住时既不会误判进球，也不会误报“非袋口失踪”；袋唇清空后若球仍未出现，异常丢失计数才恢复。共享判定器可以继续等待稍后到达袋口的视觉穿越、重现反证或轨迹释放。球被手动移走且始终没有袋口证据时，会在跟踪器释放后再连续缺失默认 `8` 帧，随后按“球在非袋口区域持续丢失”失败。每次开始、重新开始、重置或切换训练项目都会清理上一轮的共享进球候选和袋口观察历史，视觉证据不会跨轮次继承。以下情况仍会直接判定本次失败：进错号码、黑八过早进入、白球落袋、多球同时确认进袋无法判断顺序、球在非袋口区域持续丢失、已判定进球的球重新出现。旧配置 `training.pocket_proximity_mm` 仅为兼容已有 YAML 保留，训练进球判定不再读取它。
