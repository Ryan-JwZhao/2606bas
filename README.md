...
进阶训练继续使用原有严格策略：进错号码、黑八过早进入、白球落袋，以及多球同时确认进袋而无法判断顺序时，本次训练失败。新手训练使用宽松策略：未进球保持当前进度；误进球记录错误并提示重摆，目标不会错误前移；白球落袋提示重摆后继续，已完成进度不清空；自由清台项目显示剩余球、已进数量和“自由选择模式”。新手布局在球心可达区域上额外检查贴库、贴球和袋口安全距离，轻微偏差会显示“摆球基本正确，可以开始”，缺球、多球、错区或明显错序则给出具体调整提示。
