...
联动校准会先显示台面中心 ChArUco 做可见性预检。预检失败时应关闭或调暗球桌照明灯，避免环境光冲淡投影黑白对比度；必要时将工业相机曝光降低1～2档。每个必要图样首次不足 6 个角点时会自动补采一次，仍不足时停留在当前图样并提示现场调整后重试，不再静默跳过直到整轮末尾才报错。四个角袋图样和两个中袋图样均向哑光台布内错开，避开袋洞、台边反光及投影裁切；全域图样也会避开高反光台边。
...
联动校准的空间覆盖门禁按实际匹配角点计算：横向覆盖至少62%、纵向至少62%、归一化凸包面积至少34%，全域和中心图样必须存在，并且至少识别到 4 个袋口区域。不会通过降低质量门槛接受空间约束不足的结果。
...
桌面顶部同时显示总体验收结论、有效图样数、匹配点总数、RANSAC 内点 mean/P95/max、宽度/高度/凸包覆盖率、袋口覆盖数量及跨图样 CV P95。判断校准是否有效应以顶部 `PASS/FAIL` 和这些量化指标为准；全部点位图用于确认空间分布和快速定位局部红点，不需要把多个 ArUco/ChArUco 图样同时投出，以免图样之间互相干扰。
