...
规则比赛检测到进球后，桌面操作端会打开约 `4.5 s` 的非模态置顶提示窗，Web 控制页也会显示醒目提示。球类缩写为：白球 `wb进球`、黑球 `bb进球`、纯色球 `sob进球`、花色球 `stb进球`；同一批多球进袋会合并显示。提示模块在 modern 的 `POCKET_DETECTED`（已通过自动确认窗口）到达时立即提示，后续同一决定的 `POCKET_CONFIRMED / POT_PROBABLE` 按决定编号去重，不会重复弹窗；候选和暂定阶段不会提示。桌面提示窗由 BAS 主控制台持有、不会阻塞实时处理或抢占键盘焦点，并在软件日志区留下“进球提示”记录。它不写入视频帧，也不调用 `ProjectionWindow`，因此投影侧不会出现这段文字。
...
第一次修复后，上游虽已依次产生 `POCKET_COMMIT_READY`、`POCKET_DETECTED`、`POCKET_CONFIRMED` 和 `POT_PROBABLE`，通知模块却只监听最后两个事件，导致该视频从实际检测进球的 102.38 秒延迟到回合结算的 108.05 秒才有机会显示；随后又发现桌面实现只是视频预览内部 QLabel，测试中的 `isVisible()` 不能证明 Windows 操作端真正出现了提示。现在 `bas/ui/pocket_notice_popup.py` 提供由主控制台持有的独立非模态提示窗，在 `POCKET_DETECTED` 到达时显示“白球进洞 / 黑八进洞 / 全色球进洞 / 花色球进洞”，并用同一 `decision_id` 抑制后续确认别名的重复弹窗。该窗口与投影窗口没有调用关系。
