...
规则比赛检测到进球后，桌面操作端的视频预览会显示约 `4.5 s` 的醒目弹层，Web 控制页同步显示提示。球类缩写为：白球 `wb进球`、黑球 `bb进球`、纯色球 `sob进球`、花色球 `stb进球`；同一批多球进袋会合并显示。提示模块在 modern 的 `POCKET_DETECTED` 到达时立即提示，后续同一决定的 `POCKET_CONFIRMED / POT_PROBABLE` 按决定编号合并。桌面弹层持续保留软件操作焦点，并在日志区留下“进球提示”记录。它只属于操作软件，不写入视频帧，投影侧保持原画面。
...
桌面操作端的独立提示窗和 Web 前端会明显显示 `wb进球`、`bb进球`、`sob进球` 或 `stb进球`。提示消费 `POCKET_DETECTED / POCKET_CONFIRMED / POT_PROBABLE` 并按进球决定去重；投影交互层不会显示进球文字。
...
桌面操作端恢复视频预览内弹层，在 `POCKET_DETECTED` 到达时显示“白球进洞 / 黑八进洞 / 全色球进洞 / 花色球进洞”，并用同一 `decision_id` 合并后续确认别名。弹层在 `4.5 s` 后自动关闭，实时处理持续运行；投影窗口保持独立。
