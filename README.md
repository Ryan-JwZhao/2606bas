...
使用步骤：先确认当前启用的球心补偿、投影标定、相机分辨率/旋转、坐标域和检测模型均未改变，再打开“校准工作台”，点击“重采 ≥5 mm 训练点”，进入向导后点击“开始重采高残差点”。向导会在日志中列出待重采的原始点号和残差。若 56 点网格、校准上下文或训练样本编号不兼容，系统会拒绝局部替换，避免混用不同硬件或坐标系的数据。
