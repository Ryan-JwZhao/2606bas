...
`training.disappearance_confirm_frames` 仍用于识别没有任何有效袋口候选的持续失踪：默认连续 `8` 帧未识别且共享判定器也没有候选证据时，才按“球在非袋口区域持续丢失”失败。以下情况仍会直接判定本次失败：进错号码、黑八过早进入、白球落袋、多球同时确认进袋无法判断顺序、球在非袋口区域持续丢失、已判定进球的球重新出现。旧配置 `training.pocket_proximity_mm` 仅为兼容已有 YAML 保留，训练进球判定不再读取它。
