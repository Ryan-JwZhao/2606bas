...
规则比赛检测到进球后，桌面操作端的视频预览和 Web 控制页都会显示约 `4.5 s` 的醒目提示。球类缩写为：白球 `wb进球`、黑球 `bb进球`、纯色球 `sob进球`、花色球 `stb进球`；同一批多球进袋会合并显示。提示模块在 modern 的 `POCKET_DETECTED`（已通过自动确认窗口）到达时立即提示，后续同一决定的 `POCKET_CONFIRMED / POT_PROBABLE` 按决定编号去重，不会重复弹窗；候选和暂定阶段不会提示。它只属于服务端操作界面和 Web 控制状态，不写入视频帧；投影交互不会把这个文字提示投影出去。
...
桌面预览和 Web 前端会明显显示 `wb进球`、`bb进球`、`sob进球` 或 `stb进球`。提示消费 `POCKET_DETECTED / POCKET_CONFIRMED / POT_PROBABLE` 并按进球决定去重；投影交互层不会显示进球文字。
...
GUI播报控件本身一直存在。第一次修复后，上游虽已依次产生 `POCKET_COMMIT_READY`、`POCKET_DETECTED`、`POCKET_CONFIRMED` 和 `POT_PROBABLE`，通知模块却只监听最后两个事件，导致该视频从实际检测进球的 102.38 秒延迟到回合结算的 108.05 秒才有机会显示。现在桌面GUI与Web端在 `POCKET_DETECTED` 到达时立即显示“白球进洞 / 黑八进洞 / 全色球进洞 / 花色球进洞”，并用同一 `decision_id` 抑制后续确认别名的重复弹窗。
