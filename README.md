...
联动校准会先显示台面中心 ChArUco 做可见性预检。预检失败时应关闭或调暗球桌照明灯，避免环境光冲淡投影黑白对比度；必要时将工业相机曝光降低1～2档。六个袋口图样会保持完整尺寸并在投影边缘自动向台内平移，全域图样也会避开高反光台边。
...
联动校准的空间覆盖门禁按实际匹配角点计算：横向覆盖至少55%、纵向至少55%、归一化凸包面积至少25%，并且全域和中心图样必须存在。袋口图样数量继续记录在质量报告中，但当全域角点已经覆盖台面时，不再因为只识别到2～3个袋口图样而机械拒绝结果。上、下两个中袋图样会分别向台布内部及左右两侧错开，避开袋洞、台边反光和照明中轴，同时补充横向约束。

每次联动校准开始时，日志会打印算法版本、覆盖门槛、中袋布局版本和实际加载的 `linked.py` 绝对路径。当前版本应显示 `linked-geometry-v5` 与 `coverage=55%/55%/25%`。主窗口关闭时会显式退出 Qt 应用，防止投影窗口或隐藏子窗口让旧 Python 进程继续驻留。
