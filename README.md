## 2026-06-29 球杆蓄力防翻线与前端候选框

### 简介

- 规则模式下新增了“球杆方向翻面抑制”。
- 当球杆已经瞄定白球、后抽或前送只是在杆轴方向上来回运动时，系统会把这种变化识别为“蓄力动作”而不是“重新换目标”，避免路线在前后抽动时来回切线。
- 这个修复只拦截“方向反向翻面”的异常情况，正常的前向微调和真实转向仍然可以继续切线。
- 主界面新增“显示球杆矩形候选框”开关，用于在前端预览里显示球杆走廊矩形和候选球，方便现场排查。
- 候选框只画在前端预览层，不会进入投影 overlay，不会打到球台上。
...
2. 使用规则模式瞄准白球并进入蓄力；如果只是后抽/前送，路线会保持当前前方目标，不再因为杆体抽动而跳到另一条线。
3. 如果需要排查球杆走廊命中范围，在左侧勾选“显示球杆矩形候选框”。
4. 观察前端预览中的矩形走廊、候选球小框和状态文字；这些调试图形不会出现在投影窗口里。

# BAS 本地说明
## 2026-06-29 状态机与球杆方向修复
- 当前回合目标组现在统一由 `bas/state/targeting.py` 解析：人工切换、裁判账本、稳定可见计数会先合成为一个输出目标，再交给规划器使用。
- 当当前目标组已经连续稳定不可见，且黑球仍在台面上时，系统会把输出目标提升为 `black`，解决实战中某组球明显清完后仍不进入黑球回合的问题；该判断不会直接改写账本，调试快照会保留 `target_resolution` 原因。
- Stream Deck / 远程切换目标组后，主界面会立即用状态机里的最新目标组刷新路线，不再等下一帧才可能生效。
- 规则模式和自由模式共用 `bas/planning/cue_aim.py` 的球杆瞄准线检测：优先从当前画面边缘识别真实杆线，失败时再回退到 `cue_stick` 检测框，改善斜杆或检测框方向不可靠时球杆方向纠正不生效的问题。
- 球杆纠正现在使用“沿球杆方向发出的矩形走廊”：只限制目标球到球杆中心线的横向像素距离，前向长度不限制。默认总宽度为 `planner.cue_sector_corridor_width_px=140`。
- 球杆矩形走廊切换会在确认帧数内持有上一条仍然有效的路线；如果杆线短时抖到对方球方向，调试信息会显示 `cue_sector_policy=stable_hold`，不会立即把画线切过去。
...
2. 在主界面“设置 -> 路线策略”确认“启用球杆矩形走廊纠正”已开启。
3. 现场调矩形范围时改“矩形总宽度(px)”；如果仍闪烁，优先增大“切换确认连续帧”，其次增大“切换最小分差”。
4. 现场可用 Stream Deck 的 `toggle-target-group` 在 `solid` / `stripe` 之间切换当前目标组；若当前组已稳定清零，系统会自动推荐黑球。
5. 排查时查看状态调试信息里的 `turn_target_group`、`raw_turn_target_group`、`target_resolution`、`visible_group_counts`、`cue_sector_geometry` 和 `cue_sector_policy`。

## 2026-06-28 设置窗口分组优化
- 设置窗口不再把全部参数集中在一个“基础”页里，已拆成 `相机采集`、`检测状态`、`标定几何`、`投影输出`、`路线策略`、`学习数据`、`投影修正`、`颗星公式` 等子标签页。
- 常用现场调参入口更聚焦：相机相关参数在 `相机采集`，模型/状态机在 `检测状态`，路径与标定文件在 `标定几何`，路线冻结和球杆矩形走廊纠正在 `路线策略`。
...
2. 点击主界面“设置”。
3. 按需要切换到对应子标签页修改参数，点击“保存”后会写入 `local_settings/user_settings.json`。
4. `local_settings/`、日志、录像、模型权重和其它大文件已经由 `.gitignore` 排除，不会误提交到 Git。

## 2026-06-28 球杆矩形走廊纠正
- 规则模式下的球杆方向纠正已经改为“矩形走廊”：当系统没有检测到有效球杆，或球杆没有指向白球时，进洞路线继续使用原有算路逻辑，不做额外干预。
- 当有效球杆进入 outline 并指向白球后，系统会以“球杆方向穿过白球中心”的方向作为中心线，只筛选位于中心线前方、且横向像素距离不超过半个走廊宽度的目标球。默认总宽度 `140px`。
- 该模型不再使用角度扇形，也不再使用“边缘剔除(度)”；现场要扩大或收窄命中范围时，直接调整 `planner.cue_sector_corridor_width_px` 或设置窗口里的“矩形总宽度(px)”。
- 默认只有所有球都静止时才启用矩形走廊纠正；静止判断只检查 `cue/solid/stripe/black`，不要求 `cue_stick` 静止，方便球杆进入瞄准时实时触发。
- 如果当前回合是纯色/花色：
  - 走廊内有己方球时，只在己方球里按进洞评分推荐。
  - 走廊内没有己方球、但有黑球时，推荐黑球。
  - 走廊内只有对方球时，临时推荐对方球，并进入二次确认：出杆后的第一碰如果确实是这颗球，才把状态机回合目标更新为该球组；如果第一碰不是它，则不修改状态机，下一杆仍按原回合重新判断。
...
2. 使用规则模式画线。
3. 打开“设置 -> 路线策略 -> 球杆矩形走廊纠正”，可调整：
   - `启用球杆矩形走廊纠正`
   - `矩形总宽度(px)`：目标球中心到球杆中心线的横向距离必须小于等于该值的一半；前向长度不限制。
   - `只在所有球静止时生效`：默认开启；关闭后球未完全静止时也允许矩形走廊纠正。
   - `静止阈值(mm/s)` / `静止阈值(px/s)`：分别用于已标定速度和未标定速度的球静止判断。
...
- 该功能可在“设置 -> 路线策略 -> 路线防闪烁”中手动开启或关闭，修改后会保存到 `local_settings/user_settings.json`。
- 目前提供以下可量化参数，便于现场调参：
