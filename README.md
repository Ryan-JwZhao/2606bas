...
`bas/planning/rail_shot.py` 是独立的贴库后备模块；自动规则/勾球模式只要全局还存在任何普通路线，就完全不把贴库候选送入排序。目标球必须位于约 `1.15` 个球直径的库边带内，第一段必须沿相邻库边，目标袋只能是该库边相邻角袋，袋口导向转角不得超过 `60°`；幽灵球、全程边界、原始阻挡球坐标、完整袋颚净空仍逐项检查。贴库候选风险下限为 `0.78`。视频回放中第一杆10/10个预击球帧有普通安全路线；第二杆锁定后的14/14帧有明确标注的高风险贴库辅助路线，而实际撞颚反弹仍由进球状态机拒绝。

GUI播报控件本身一直存在，原问题是第一杆只到 `POCKET_TENTATIVE`，随后被旧袋唇计时和观察总超时抢先否决，上游从未发出播报事件。修复后，同一确定性回放依次产生一次 `POCKET_COMMIT_READY`、`POCKET_DETECTED`、`POCKET_CONFIRMED` 和 `POT_PROBABLE`；桌面GUI与Web端共享去重后的确认通知，并显示“白球进洞 / 黑八进洞 / 全色球进洞 / 花色球进洞”。

设置入口为“设置 → 路线策略 → 入袋球心走廊 / 贴库球”。现场一般只需保持默认 `2 mm / 50° / 18° / 60°`；继续放宽最大入袋角或贴库导向转角会显著增加擦颚失败路线。
