...
- 稳定视觉球数低于账本时，单靠“没看见”不会永久扣球。存在袋口/丢失支持事件时，可把观测数作为当杆规划的有效视图，但账本仍等待最终进洞证据。
...
现场录制的 `no_line_video_*.mp4` 可以作为确定性模拟输入，按视频自身帧率依次经过真实的检测、跟踪、状态机、规划和投影链路。诊断器会显式指定目标花色，使路线代码被触发，并检查稳定状态下球圈集合、路线图元和最终亚像素绘制结果是否出现单帧 `A-B-A` 闪回；同一份报告还会列出台面球心可达区越界样本、轨迹消失位置、袋口视觉证据和进洞事件。
...
默认报告写入 `.tmp/overlay_video_diagnosis/`。报告和视频均为本地大文件/诊断产物，已经由 `.gitignore` 排除。`PASS` 表示本次回放未检测到稳定状态下的单帧球圈或路线闪回；它不等同于人工标注的进洞准确率，离线验收仍应结合报告事件、轨迹消失位置和原视频标注核验。
