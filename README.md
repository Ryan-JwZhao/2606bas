...
### 长视频录制与安全停止（2026-08-25）

主界面的“无画线视频”和“进洞路线画线视频”按钮会把 H.264 MP4 保存到 `local_settings/captures/`。再次点击对应按钮即可停止录制。停止阶段会关闭 FFmpeg 输入并持续等待末尾 `moov` 索引写完；长录像可能需要若干秒，请等待日志显示“视频已保存”后再关闭程序。

实时录制直接生成普通 MP4，并在文件末尾写入索引，省去 `+faststart` 对整段视频数据的二次搬移。这样可以让数小时、十余 GB 的录像保持短暂且稳定的收尾过程。离线拼接仍可在完整输入上执行 faststart，方便成品文件快速起播。

录制收尾回归测试：

```powershell
.\.venv\Scripts\python.exe -m pytest -q tests\test_media_capture.py
```
...
规划器不再把目标球当作没有体积的点。运行时会从每个袋口的原始标注曲线中提取两侧袋角，并将“袋心 + 两侧袋角”作为同索引的完整规划几何。2026-08-21 起，规则模式、目标球模式和勾球模式不再强迫所有球瞄准单一拟合袋心，而是在袋口两端各扣除“标准球半径 + 2 mm”后形成可通过球心走廊，并在走廊内搜索安全入袋点；最终路线到两侧袋角的最小距离仍必须覆盖完整球半径，入口角不得超过 50°。任一条件不满足时，候选在评分和投影绘制前直接丢弃。
...
### 六袋低帧率进球检测修复（2026-08-25）

进球观察器现在把帧间运动和背景前景统一投影到袋口局部坐标，并以可撤销的终端穿越证据覆盖低帧率快速进球。球在连续两帧之间从袋口前方直接消失时，只要视觉运动沿袋口方向推进到终端走廊、关联到唯一的最近消失球，并持续通过重现与反向运动校验，状态机即可形成内部检测；桌面与 Web 等待回合不可撤销确认后播报。
...
```powershell
.\.venv\Scripts\python.exe -m pytest -q tests\test_pocket_observer.py tests\test_pocket_visual_state.py tests\test_pocket_evaluator.py --basetemp=.tmp/pytest-pocket-detection
```

本机聚焦视频验收使用 `tests/fixtures/pocket_video_20260824_fixed_labels.json`，覆盖六袋真实进球、右下两次原成功样本、五次历史漏报，以及左上袋唇停球反证。该文件使用部分标签模式，负责固定已人工核对杆次；整段错报率仍需完整逐杆标注评估：

```powershell
.\.venv\Scripts\python.exe scripts\evaluate_pocket_video.py --replay replays\session_20260824_175528_494735 --video local_settings\captures\no_line_video_20260824_183732_fixed.mp4 --labels tests\fixtures\pocket_video_20260824_fixed_labels.json --stop-frame 53230
```
...
规则比赛确认进球后，桌面操作端的视频预览会显示约 `4.5 s` 的醒目弹层，Web 控制页同步显示提示。球类缩写为：白球 `wb进球`、黑球 `bb进球`、纯色球 `sob进球`、花色球 `stb进球`；同一批多球进袋会合并显示。`POCKET_DETECTED` 保留为可撤销内部状态，提示模块等待 `POCKET_CONFIRMED / POT_PROBABLE` 后按决定编号播报，避免稳定可见反证到达前产生错报。桌面弹层持续保留软件操作焦点，并在日志区留下“进球提示”记录。它只属于操作软件，不写入视频帧，投影侧保持原画面。
...
- 进入 `THROAT/INTERIOR`、可靠袋口视觉向内穿越仍是直接强证据。高速球若在严格分区前消失，只有“同一轨迹高速入袋预测 + 袋唇关联运动 + 帧差/球尺度运动/前景三类证据 + 无重现或反向”全部成立才可在反证窗口后确认；仅靠预计轨迹、袋口静止后消失或球组跨 ID 改变仍会产生 `POCKET_REJECTED`，不扣账本。
...
右上、左下角曾出现的大偏移来自袋口曲线先被 10 mm rail inset 和矩形裁剪非刚性压扁，再对压扁后的短圆弧重新拟合；这会把 10 mm 边界调整放大成超过 100 mm 的伪圆心偏移。修复后六袋规划统一使用原始拟合中心，不按像素阈值只特判两个角。`ShotCandidate.pocket_point` 仍表示洞心；普通物体球线可以终止在经完整球净空验证的袋口走廊出口，因此画线不再为了连接数学洞心而切过袋颚。
...
2. 候选进入袋口后等待至少 `1.3 s`。期间若出现向外穿越、桌面侧重现、轨迹反向或持续的新袋唇占用证据，则自动产生 `POCKET_REJECTED`。高速同轨候选允许额外等待背景模型恢复，但旧袋唇状态和纯前景残影不能重新武装否决。
3. 只有逻辑球已经持续离开桌面且没有反证的强候选才进入提交准备；球仍处于 `visible` 时，即使袋口视觉短暂误判为 `clear` 也不得确认。单纯跟踪器 `occluded` 外推不能确认；严格高速分支还必须同时满足轨迹、速度、横向捕获宽度、同轨关联和三类独立视觉证据。
...
桌面操作端的独立提示窗和 Web 前端会明显显示 `wb进球`、`bb进球`、`sob进球` 或 `stb进球`。提示只消费不可撤销的 `POCKET_CONFIRMED / POT_PROBABLE` 并按进球决定去重；投影交互层不会显示进球文字。
...
球杆方向现在使用“球杆无向轴线 → 白球侧向消歧 → 全消费者统一时序稳定”的完整链路。帧边缘候选分别计算无限轴线到白球中心的垂直距离和杆尖到白球的距离，真实球杆存在合理杆尖间隙时仍可通过，靠近白球但轴线偏离的库边、投影网格和阴影直线会被拒绝。存在 `cue_stick` 检测 track 时，边缘线还必须与该检测区域形成足够长的重叠；分割模型提供球杆 mask 时，检测层会直接用 PCA 拟合斜杆主轴并通过 tracker 传给规划器。

正反方向按球杆主体位于白球哪一侧判定。白球两侧只有几个像素差异时保持 `body_side_ambiguous`，该帧不会成为错误方向种子。方向稳定器对每次角度变化统一应用角度门槛和确认帧数，单帧 90° 干扰会保持上一方向；180° 的 Hough 端点交换继续按同一无向轴处理。稳定后的当帧方向由 `TargetShotMode`、`TargetLock`、`CueSector` 和预览箭头共同使用。

实际录像 `local_settings/captures/no_line_video_20260818_184518.mp4` 的 510–590 帧用于连续方向验收。旧算法在第 527 帧从约 `-28°` 单帧跳到 `-180°`；修复后该帧为 `-23.0°`，与人工标注方向点积为 `0.995`。整段 81 帧中检测到 80 帧，原始方向和稳定方向的错误帧均为 0；另外覆盖两个不同方向的真实球杆帧及四个“球杆远离白球”拒绝帧。

离线复验命令：

```powershell
.\.venv\Scripts\python.exe scripts\diagnose_cue_direction_video.py `
  --video local_settings\captures\no_line_video_20260818_184518.mp4 `
  --annotations tests\fixtures\cue_direction_video_20260818_labels.json
```

报告和方向箭头预览默认写入 `.tmp/cue_direction_video_diagnosis/`。预览图中绿色为人工期望方向、青色为原始测量、紫色为稳定后方向。录像、报告、预览图和检测缓存均由现有 `.gitignore` 的媒体与 `.tmp/` 规则排除。
...
## 球圈与连续击球路线稳定（2026-08-20）

本轮修复针对实时预览和实际投影中同时出现的长击球线大幅晃动。现场 `no_line_video_20260820_153411.mp4` 的 338 个有路线帧始终是目标球 #2、袋口 #1，并不存在候选路线反复切换。根因是目标球规划球心最多约 4 mm 的检测/跟踪波动被反向延伸至库边的长射线放大：旧的 `0.35 px` 显示死区只会拦截亚像素变化，超过阈值就整点放行，因此无法处理约 11.77 px 的实际投影端点摆幅。

新实现位于 `bas/planning/route_stability.py`，遵守以下规则：

- 原始检测、跟踪结果和状态机数据不修改；稳定只发生在台面毫米坐标的规划输入层。
- `STABLE_IDLE / PRE_SHOT_ARMED` 使用强抑噪和非零软响应；慢速运动逐步减弱平滑，`SHOT_ACTIVE` 等运动阶段直接使用低延迟参数和最多 `70 ms / 12 mm` 的受限预测。所有阶段都逐帧调用规划器，不隐藏路线，也不复用旧坐标。
- 每个有效帧都会从当前规划球心重新计算幽灵球、瞄准线、物体球线、碰库点、袋口净空和白球分离线。预览与投影消费同一个当帧 `ShotPlan / ProjectionOverlay`。
- 拓扑连续性只保持目标球、袋口和碰库序列。它只能从本帧已经通过碰撞与袋口净空检查的候选中选择；旧拓扑失效时当帧清除或切换，绝不继续显示旧路线。
- 平滑后的球心只用于构造连贯路线；其他阻挡球的碰撞净空始终使用本帧原始测量位置，避免平滑延迟放过已经挡住路线的球。
- GUI 预览和投影渲染不再各自保存路线端点历史。旧的 `route_freeze_*` 设置仅为读取旧配置保留，运行时不再执行整条路线冻结；亚像素 `shift=8` 绘制继续保留。
- 检测球圈、检测框和置信度文字仍可使用独立的显示级小死区，它们不参与路线物理计算。

默认参数位于 `configs/default.yaml` 的 `route_stability_*` 和 `route_topology_*`。主界面“设置 → 路线策略 → 连续路线稳定”可调整开关、静止/运动时间常数、静止速度阈值、软区和拓扑切换门槛。修改后开始新采集即可生效；正在运行时保存设置会主动清空旧稳定状态。

回归使用录像第 27–62 帧的真实球心数据，并覆盖慢移、快移、急停、大位移重置、时间回退、拓扑切换和失效路线立即清除。完整 338 帧离线几何测量中，长线端点表面坐标最大摆幅由约 `26.53 mm` 降至 `0.82 mm`；16 颗球的稳定计算在当前开发机约为 `0.41 ms/帧`。

现场建议让球静置 10 秒确认长线稳定，再分别缓慢和快速移动白球/目标球，确认路线在运动全过程持续出现并连续跟随。若球心存在稳定的同方向偏移，应检查投影或工程球心校准，不应继续增大稳定参数。

## 入袋角、贴库球与进球播报（2026-08-21）

现场 `no_line_video_20260820_183820_1.mp4` 证明旧的“目标球直连拟合袋心”标准过紧：01:39 左库实色球实际进洞，但旧模型仍计算为袋颚净空 `-16.31 mm`。新标准由 `bas/planning/pocket_clearance.py` 统一实现：袋口保留完整球半径和默认 `2 mm` 余量，在剩余球心走廊内搜索入口，入口角硬上限为 `50°`。该实进球的新净空为 `+13.12 mm`、入口角约 `27.33°`；此前 `no_line_video_20260820_153411.mp4` 的绿球侧袋坏路线即使换入口仍约 `60.44°`，继续拒绝。

贴库球不再使用“沿库边到袋前漏斗点、再折向袋心”的后备模型。`no_line_video_20260820_183820_1.mp4` 约 01:55 的蓝色纯色球证明这种三点线会制造没有碰库、碰球依据的自行转向，物理上不成立。现在非撞库目标球路线必须全程为单一直线，并通过完整球心走廊；第一杆实际进洞的左库实色球仍由该直线标准接受。只有目标击球/勾球模式明确计算出真实碰库点时，目标球线才允许折线，碰库次数和库边也会写入路线说明。

桌面操作端恢复视频预览内弹层，在 `POCKET_CONFIRMED / POT_PROBABLE` 到达时显示“白球进洞 / 黑八进洞 / 全色球进洞 / 花色球进洞”，并用同一 `decision_id` 合并确认别名。弹层在 `4.5 s` 后自动关闭，实时处理持续运行；投影窗口保持独立。

完整从第 0 帧回放 `no_line_video_20260820_183820_1.mp4` 后，前两分钟的三次真实进球分别在 `16.66 s`、`94.75 s`、`101.23 s` 产生操作端提示，对应轨迹 `9 / 73 / 4`，三次决定均在回合结算确认。统一证据模型覆盖预击球阶段袋唇离开、高速跨 ID 接力、窄通道终点消失三种情况；每种情况共用袋口几何、速度上限、运动来源、持续消失和重现校验。`112.91 s` 的撞颚反弹保持拒绝。

设置入口为“设置 → 路线策略 → 入袋球心走廊”。现场一般只需保持默认 `2 mm / 50°`；旧版用户设置里的贴库方向和袋口转角字段会被兼容忽略，避免继续启用不物理的袋口折向。

### 无画线视频离线闪烁诊断

现场录制的 `no_line_video_*.mp4` 可以作为确定性模拟输入，按视频自身帧率依次经过真实的检测、跟踪、状态机、连续路线稳定、规划和投影链路。诊断器不会再模拟旧的整条路线冻结或独立端点显示死区；默认也不注入目标花色，开台、分组和回合状态完全使用状态机的实际判断。报告会检查稳定状态下球圈集合、路线图元和最终亚像素绘制结果是否出现单帧 `A-B-A` 闪回，并列出台面球心可达区越界样本、轨迹消失位置、袋口视觉证据和进洞事件。
