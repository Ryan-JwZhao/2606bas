...
规则比赛检测到进球后，桌面操作端的视频预览会显示约 `4.5 s` 的醒目弹层，Web 控制页同步显示提示。球类缩写为：白球 `wb进球`、黑球 `bb进球`、纯色球 `sob进球`、花色球 `stb进球`；同一批多球进袋会合并显示。`POCKET_DETECTED` 到达时立即提示，并立即更新用于路线规划和界面显示的临时目标花色；后续同决定的 `POCKET_CONFIRMED / POT_PROBABLE` 按决定编号去重并由正式账本接管，`POCKET_REJECTED` 会回滚临时花色并显示对应球型的“判定撤销”。已有弹层仍在显示时，新提示会先隐藏约 `90 ms` 再重新出现，形成明显闪烁并重启显示计时。正式演员花色、库存和裁判结果仍在 `TURN_RESOLVE` 后提交。弹层只属于操作软件，不写入视频帧，投影侧保持原画面。
...
桌面操作端的独立提示窗和 Web 前端会明显显示 `wb进球`、`bb进球`、`sob进球` 或 `stb进球`。提示消费即时 `POCKET_DETECTED`，按决定编号合并后续确认别名，并显示 `POCKET_REJECTED` 撤销消息；投影交互层不会显示进球文字。
...
桌面操作端恢复视频预览内弹层，在 `POCKET_DETECTED` 到达时显示“白球进洞 / 黑八进洞 / 全色球进洞 / 花色球进洞”，并用同一 `decision_id` 合并后续确认别名。新提示到达时会强制闪烁刷新；撤销决定会显示“判定撤销”。弹层在 `4.5 s` 后自动关闭，实时处理持续运行；投影窗口保持独立。
