...
校准工作台的“球心补偿”区域新增“重采 ≥5 mm 训练点”按钮。该入口读取当前启用的 56 点球心补偿文件，使用与运行时一致的球心平面映射和样本权重重新计算每个训练点残差，只安排残差不低于 `5.0 mm` 的点重采。新观测按原 `sample_index` 替换旧样本，其余训练样本保持不变；旧 Holdout 会被清空，完成定点重采后必须重新采集一套全新的 30 次 Holdout。
